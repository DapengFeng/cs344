...
__global__ 
void scan_kernel(unsigned int* d_bins, int size) { // it allows multi-blocks but denys shared memory on the device
    int mid = threadIdx.x + blockDim.x * blockIdx.x;
    if(mid >= size)
        return;
...
__global__
void reduce_minmax_kernel(const float* const d_in, float* d_out, const size_t size, bool minmax) {
...
float reduce_minmax(const float* const d_in, const size_t size, bool minmax) {
    int BLOCK_SIZE = 1024;
